...
def get_features(train_features, train_labels, test_features, test_labels):
    ...
    depth, num_example = CART(train_features,train_labels, test_features, test_labels)
    dic = {"Depth":depth, "number of samples":num_example, "distance to nearest neighbour":dist}
    dic.update(bf)
    return dic
...
mse_rf = mean_squared_error(test_labels, rf.predict(test_features))
...
mse_svr = mean_squared_error(test_labels, svr_rbf.predict(test_features))

#PPR
estimator = ProjectionPursuitRegressor()
estimator.fit(train_features,train_labels)
ppr_pred = estimator.predict(test_features)
mse_ppr = mean_squared_error(test_labels, estimator.predict(test_features))


#GB
reg = ensemble.GradientBoostingRegressor()
reg.fit(train_features,train_labels)
mse_gb = mean_squared_error(test_labels, reg.predict(test_features))

dist_ppr = np.linalg.norm(ppr_pred-test_labels)
dist_svr = np.linalg.norm(svr_pred-test_labels)
dist_rf = np.linalg.norm(rf_predict-test_labels)
# Random Noise
...
print(model.trace())
print(model.summary())
prediction_mars = model.predict(test_features)
...
n_nodes_ = [t.tree_.node_count for t in rf.estimators_]
left      = rf.tree_.children_left
max_depth = list()
for tree in rf.estimators_:
    max_depth.append(tree.tree_.max_depth)
n_leaves = np.zeros(1000, dtype=int)
for i in range(1000):
    n_nodes = rf.estimators_[i].tree_.node_count
    # use left or right children as you want 
    children_left = rf.estimators_[i].tree_.children_left
